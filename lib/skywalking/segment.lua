...
function _M.transform(segment)
    local segmentBuilder = Util.tablepool_fetch("sw_segmentBuilder", 0, 8)
    ...
    segmentBuilder.spans = Util.tablepool_fetch("sw_segmentBuilder_spans", 8, 0)

    if segment.spans ~= nil and #segment.spans > 0 then
        for _, span in ipairs(segment.spans) do
            insert_tab(segmentBuilder.spans, Span.transform(span))
        end
    end
